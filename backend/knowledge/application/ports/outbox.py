...
class KnowledgeOutboxPort(Protocol):
    """Transactional outbox port for knowledge domain events.

    When a source, document, chunk, or ingestion aggregate changes
    state, the corresponding domain event is stored in an outbox so
    it can be published to NATS (or another message broker) reliably.
    The outbox ensures at-least-once delivery semantics.
    """
    ...
    def mark_published(self, aggregate_id: str) -> None:
        ...
        aggregate_id:
            ...
            representation of ``KnowledgeSourceId``, ``DocumentId``,
            ``ChunkId``, or ``IngestionJobId``).
        """
        ...
