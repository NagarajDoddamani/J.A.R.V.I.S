...
class PlannerOutboxPort(Protocol):
    """Transactional outbox port for planner domain events.

    When a plan or task aggregate changes state, the corresponding
    domain event is stored in an outbox so it can be published to
    NATS (or another message broker) reliably. The outbox ensures
    ...
    def mark_published(self, aggregate_id: str) -> None:
        ...
        aggregate_id:
            ...
            representation of ``PlanId`` or ``TaskId``).
        """
        ...
